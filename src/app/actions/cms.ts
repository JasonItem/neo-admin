...
export async function saveCmsSiteSettingsAction(input: unknown) {
  ...
  const [existing] = await db
    .select({ id: cmsSiteSettings.id })
    .from(cmsSiteSettings)
    .where(eq(cmsSiteSettings.tenantId, actor.tenantId))
    .limit(1);
  const values = {
    ...parsed,
  ...
  if (existing)
    ...
    await db
      ...
      .values({ id: siteId, tenantId: actor.tenantId, ...values });
...
export async function uploadCmsMediaAction(formData: FormData) {
  ...
  const altText = z
    ...
    .trim()
    .max(255)
    .parse(formData.get("altText") ?? "");
  if (!(file instanceof File) || !file.size)
    throw new Error("请选择需要上传的文件");
  const extension = allowedTypes.get(file.type);
  if (!extension)
    throw new Error("仅支持图片、MP4/WebM/MOV 视频、PDF、Word 和 Excel 文件");
...
export async function setCmsLogoAction(mediaId: string) {
  const actor = await requirePermission(PERMISSIONS.cmsSiteUpdate);
  const [media] = await db
    .select({ id: cmsMedia.id, mimeType: cmsMedia.mimeType })
    .from(cmsMedia)
    .where(
      and(
  ...
  if (!media || !media.mimeType.startsWith("image/"))
    throw new Error("只能将图片设为网站 Logo");
  ...
  if (settings)
    await db
      ...
      .where(eq(cmsSiteSettings.id, settings.id));
  else
    await db
      .insert(cmsSiteSettings)
      .values({
...
export async function deleteCmsMediaAction(mediaId: string) {
  ...
  const [site] = await db
    ...
    .where(
      and(
        eq(cmsSiteSettings.tenantId, actor.tenantId),
        eq(cmsSiteSettings.logoMediaId, media.id),
      ),
    )
    .limit(1);
