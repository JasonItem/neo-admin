...
const renderIcon = (item: NavigationItem) =>
  React.createElement(iconFor(item), { className: "size-4" });
const containsPath = (item: NavigationItem, pathname: string): boolean =>
  item.path === pathname ||
  item.children.some((child) => containsPath(child, pathname));
  ...
  if (item.children.length)
    return (
      <DropdownMenuSub>
        ...
        <DropdownMenuSubContent>
          {item.path && (
            <DropdownMenuItem render={<Link href={item.path} />}>
              {renderIcon(item)}
              {item.name}
            </DropdownMenuItem>
          )}
...
function RootMenuItem({
  ...
  const menuClassName =
    "h-10 shrink-0 gap-2 rounded-lg px-3 text-sm font-medium";
  if (!item.children.length)
    return (
      <Button
        variant="ghost"
...
function TopMenu({ items }: { items: NavigationItem[] }) {
  ...
  const visibleCount = React.useMemo(() => {
    ...
    if (itemWidths.reduce((sum, value) => sum + value, 0) <= width)
      return items.length;
    let used = 44;
    let count = 0;
  ...
  return (
    <nav
      ref={ref}
      className="ml-2 hidden min-w-0 flex-1 items-center gap-1 overflow-hidden md:flex"
    >
      {visible.map((item) => (
        <RootMenuItem key={item.id} item={item} pathname={pathname} />
      ))}
      {overflow.length > 0 && (
        <DropdownMenu>
          <DropdownMenuTrigger
...
export function TopNavigation({
  ...
  return (
    <div className="border-b bg-background">
      <div className="mx-auto flex h-14 max-w-[1500px] items-center gap-2 px-4 md:px-6 lg:px-8 2xl:px-12">
        <DropdownMenu>
          <DropdownMenuTrigger
            render={
              <Button variant="ghost" className="h-11 shrink-0 gap-2 px-2" />
            }
          >
