...
import { menuItems, organizations, roleMenuItems, roles, tenants, userRoles, users } from "./schema";
import { PERMISSION_NAMES, PERMISSIONS, type PermissionCode } from "../lib/permissions";
...
async function main() {
  ...
  await db.insert(tenants).values({ id: ids.org, name: "总部", code: "HQ" }).onDuplicateKeyUpdate({ set: { name: "总部" } });
  await db.insert(organizations).values({ id: ids.org, tenantId: ids.org, name: "总部", code: "HQ", type: "COMPANY", path: ids.org, sortOrder: 1 }).onDuplicateKeyUpdate({ set: { name: "总部" } });
  await db.insert(roles).values({ id: ids.role, tenantId: null, name: "超级管理员", code: "SUPER_ADMIN", defaultDataScope: "PLATFORM", builtIn: true }).onDuplicateKeyUpdate({ set: { name: "超级管理员", defaultDataScope: "PLATFORM" } });
  const [existing] = await db.select({ id: users.id }).from(users).where(eq(users.username, username)).limit(1);
  const userId = existing?.id ?? ids.admin;
  if (!existing) await db.insert(users).values({ id: userId, tenantId: ids.org, organizationId: ids.org, username, passwordHash: await hash(password, 12), displayName });
  await db.insert(userRoles).values({ userId, roleId: ids.role, tenantId: ids.org, anchorOrganizationId: ids.org }).onDuplicateKeyUpdate({ set: { tenantId: ids.org, anchorOrganizationId: ids.org } });
  ...
  const definitions = [
    ...
    { id: ids.orgs, parentId: ids.system, name: "组织管理", type: "MENU" as const, path: "/system/organizations", icon: "Building2", permissionCode: PERMISSIONS.organizationList, sortOrder: 4 },
    { id: ids.operationLogs, parentId: ids.system, name: "操作日志", type: "MENU" as const, path: "/system/operation-logs", icon: "FileClock", permissionCode: PERMISSIONS.operationLogList, sortOrder: 5 },
  ...
  const buttonParents = [
    ...
    [ids.orgs, [PERMISSIONS.organizationCreate, PERMISSIONS.organizationUpdate, PERMISSIONS.organizationDelete]],
    [ids.system, [PERMISSIONS.profileUpdate, PERMISSIONS.passwordUpdate]],
  ] as const;
  const allDefinitions: Array<(typeof definitions)[number] | { id: string; parentId: string; name: string; type: "BUTTON"; path: null; icon: null; permissionCode: string; sortOrder: number; visible: boolean }> = [...definitions];
  let sequence = 100;
  for (const [parentId, permissions] of buttonParents) for (const permissionCode of permissions) allDefinitions.push({ id: `20000000-0000-4000-8000-${String(sequence++).padStart(12, "0")}`, parentId, name: PERMISSION_NAMES[permissionCode as PermissionCode], type: "BUTTON", path: null, icon: null, permissionCode, sortOrder: sequence, visible: false });
  for (const definition of allDefinitions) {
    const [existingMenuItem] = definition.permissionCode
      ? await db.select({ id: menuItems.id }).from(menuItems).where(eq(menuItems.permissionCode, definition.permissionCode)).limit(1)
      : [];
